---
sidebar_label: twturbiks_starter ORM 相關教學
sidebar_position: 4
---
...
[odoo 系统中 name_search 和 name_get 用法](https://blog.51cto.com/melon0809/5389459)
...
Odoo shell，也被稱為 Odoo 命令行界面（CLI），是由 Odoo 提供的一個命令行工具，允許您使用 Python 命令與 Odoo 實例進行交互。它提供了一種執行 Python 代碼並與 Odoo 環境進行交互的方式，包括訪問和操作數據庫中的數據，運行查詢以及執行各種管理任務。
