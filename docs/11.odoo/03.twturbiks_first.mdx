...
[odoo14 mail.thread 邮件消息机制（发送消息、字段变化跟踪记录）mail.activity.mixin 安排活动](https://blog.csdn.net/weixin_44863237/article/details/123736932)
...
> computed_field 1. 預設是 readonly, 2.不存在 db 中 (store=False), 3.搜尋 field_compute_demo 時, 會發現錯誤,
...
model*id:id 格式 model*{model \_name}
...
3. tests 資料夾底下的 testing 都必須是 test\_ 開頭的,
